...
parser.add_argument('--dataset', metavar='DATASET', default='cifar10',
                        ...
                        help='dataset: cifar10 or svhn or cifar100' )
...
parser.add_argument('--rdropalpha',default=0.1,type=float,help='rdrop hyperparameter')
parser.add_argument('--rdrop_rampup_starts', default=30, type=int, metavar='EPOCHS',
                    help='epoch at which rdrop loss ramp-up starts')
parser.add_argument('--rdrop_rampup_ends', default=30, type=int, metavar='EPOCHS',
                    help='lepoch at which rdrop loss ramp-up ends')
...
if args.dataset == 'cifar100':
    data_source_dir = args.data_dir
    trainloader, validloader, unlabelledloader, testloader, num_classes = load_data_subset(1, args.batch_size, args.n_cpus ,'cifar100', data_source_dir, labels_per_class = args.num_labeled, valid_labels_per_class = args.num_valid_samples)
...
def experiment_name(sl = False,
    ...
    exp_name += '_time_'+ time.strftime("%Y-%m-%d_%H",time.localtime())
    ...
    exp_name += '_rdropalpha_'+str(rdropalpha)
...
def rdrop_loss(logits, newlogits, num_classes=10):
    '''the KL loss between two logits output'''
    p = torch.log_softmax(logits.view(-1, num_classes), dim=-1)
    p_tec = torch.softmax(logits.view(-1, num_classes), dim=-1)
    q = torch.log_softmax(newlogits.view(-1, num_classes), dim=-1)
    q_tec = torch.softmax(newlogits.view(-1, num_classes), dim=-1)
...
def get_current_rdrop_weight(final_rdrop_weight, epoch, step_in_epoch, total_steps_in_epoch):
    # Consistency ramp-up from https://arxiv.org/abs/1610.02242
    epoch = epoch - args.rdrop_rampup_starts
    epoch = epoch + step_in_epoch / total_steps_in_epoch
    return final_rdrop_weight * ramps.sigmoid_rampup(epoch,
                                                           args.rdrop_rampup_ends - args.rdrop_rampup_starts)
...
def train(trainloader,unlabelledloader, model, ema_model, optimizer, epoch, filep):
    ...
    for (input, target), (u, _) in zip(cycle(trainloader), unlabelledloader):
        ...
        if args.mixup_sup_alpha:
            ...
            ###TODO add Rdrop in supervised loss 
            output_mixed_l_new = model(mixed_input_var)
            rdroploss = rdrop_loss(output_mixed_l,output_mixed_l_new,num_classes) 
            ###
            # add rdrop alpha rampup
            if epoch < args.rdrop_rampup_starts:
                rdropalpha_rampup = 0.0
            else:
                rdropalpha_rampup = get_current_rdrop_weight(args.rdropalpha, epoch, i, len(unlabelledloader)) 
            rdroploss = rdroploss * rdropalpha_rampup 
            loss_func = mixup_criterion(target_a_var, target_b_var, lam)
            class_loss = 0.5*(loss_func(class_criterion, output_mixed_l)+loss_func(class_criterion, output_mixed_l_new))
        ...
        meters.update('class_loss', class_loss.item())
        meters.update('rdrop_loss', rdroploss.item())
        ...
        if args.mixup_consistency:
                ...
                #     #TODO add Rdrop in unsupervised loss in mixup_hidden
                # else:
                    #output_u = model(u_var)
                ...
                #TODO add Rdrop in unsupervised loss 
                output_mixed_u_new = model(mixedup_x)
                rdroploss_u = rdrop_loss(output_mixed_u,output_mixed_u_new,num_classes)  
                if epoch < args.rdrop_rampup_starts:
                    rdropalpha_rampup = 0.0
                else:
                    rdropalpha_rampup = get_current_rdrop_weight(args.rdropalpha, epoch, i, len(unlabelledloader)) 
                rdroploss_u = rdroploss_u * rdropalpha_rampup
                meters.update('rdrop_cons_loss', rdroploss_u.item())

                mixup_consistency_loss = 0.5*(consistency_criterion(output_mixed_u, mixedup_target) + consistency_criterion(output_mixed_u_new, mixedup_target))/ minibatch_size# criterion_u(F.log_softmax(output_mixed_u,1), F.softmax(mixedup_target,1))
        ...
        loss = class_loss +  rdroploss +  mixup_consistency_loss + rdroploss_u
        meters.update('loss', loss.item())
        ...
        if i % args.print_freq == 0:
            ...
            print(
                ...
                'Data {meters[data_time]:.3f}\t'
                'Class {meters[class_loss]:.4f}\t'
                'Rdrop {meters[rdrop_loss]:.4f}\t'
                'Mixup Cons {meters[mixup_cons_loss]:.4f}\t'
                'Rdrop Cons {meters[rdrop_cons_loss]:.4f}\t'
                'Prec@1 {meters[top1]:.3f}\t'
